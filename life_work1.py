...
roomsm = {}  # Room details with players and listeners
...
def create_or_join_room():
    ...
    if role == 'Player' and len(roomsm[room_name]['players']) == 0:
        roomsm[room_name]['players'].append({'username': username})
    elif role == 'Listener':
        roomsm[room_name]['listeners'].append({'username': username, 'volume': 1})
    else:
...
def handle_join(data):
    ...
    if role == 'Player':
        if len(roomsm[room_name]['players']) < 1:
            roomsm[room_name]['players'].append({'id': request.sid, 'username': username})
        else:
            emit('error', {'message': 'A player already exists in this room.'})
            return
    elif role == 'Listener':
        roomsm[room_name]['listeners'].append({'id': request.sid, 'username': username, 'volume': 1})

    join_room(room_name)
    emit('update_listener_list', roomsm[room_name], room=room_name)
...
def handle_volume_update(data):
    """Handle volume updates for a listener."""
    room_name = data.get('room')
    listener_id = data.get('listenerId')
    new_volume = data.get('volume')

    if room_name in roomsm:
        for listener in roomsm[room_name]['listeners']:
            if listener['id'] == listener_id:
                listener['volume'] = new_volume
                break
...
def handle_remove_listener(data):
    ...
    if room_name in roomsm:
        rooms[room_name]['listeners'] = [
            listener for listener in roomsm[room_name]['listeners'] if listener['id'] != listener_id
        ]
        emit('update_listener_list', roomsm[room_name], room=room_name)
...
def handle_disconnect():
    ...
    for room_name, room_data in roomsm.items():
        room_data['players'] = [
            player for player in room_data['players'] if player['id'] != request.sid
        ]
        room_data['listeners'] = [
            listener for listener in room_data['listeners'] if listener['id'] != request.sid
        ]
        emit('update_listener_list', room_data, room=room_name)
